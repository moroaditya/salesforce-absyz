...
*/
    ...
    /* 
     ...
     Created By: Durga
     Purpose   : Updating the Tax Rate field on Service Quote.
     */
        if(trigger.isBefore && trigger.isInsert){  
        system.debug('The testtttt is:::');
        List<Service_Quote__c> sqList = [select id, TaxRate__c, Case__c, Shipping_State__c,RecordType.Name from Service_Quote__c where RecordType.Name = 'Custom Service Quote'];
        system.debug('The sqlist is:::'+sqList);
        SQ_TaxRates.updateTaxRateOnServiceQuote(sqList);
        system.debug('The sqlist is:::'+sqList);
        }
