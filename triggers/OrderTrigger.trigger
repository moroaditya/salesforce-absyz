...
//            nq.Order_2__c = orderidmap.get(nq.Id);
        ...
        for( Order o : trigger.new )
        {
            ...
            { //the order finalized date was set
                ...
                if( !pipelineEmails.containsKey(o.Id) 
                    ...
                    {
                        system.debug('Either no installer or no installer email found for order ' + o.Id );
                        continue;
                    }
                    
                system.debug('sending CAD email to  ' + pipelineEmails.get(o.Id) );
//Quote.Nana_Quote__r.Installer__r.Company_Email__c             
                pp_EmailFlowUtility.sendTemplatedEmail( new string[]{ o.Installer_Email__c }, new string[]{}, 'Quote_2_0_CAD_EMAIL_TEMPLATE', dummyContact.Id, o.Id,  '0D2A0000000TNUg' , false, null );//setting System as WhoId to get past stupid limitations
//              pp_EmailFlowUtility.sendTemplatedEmail( new string[]{ pipelineEmails.get(o.Id).Company_Email__c }, new string[]{}, 'Quote_2_0_CAD_EMAIL_TEMPLATE', dummyContact.Id, o.Id,  '0D2A0000000TNUg' , false, null );//setting System as WhoId to get past stupid limitations
    ...
    } else  if( trigger.isBefore && trigger.isUpdate )
    {
        ...
        for( Order o : trigger.new )
        {
            ...
            { //the order finalized date was set
                ...
                pp_EmailFlowUtility.doCallout(o.Quote.Name, hex);
            }
        }
    ...
    } else if( trigger.isBefore && trigger.isInsert )
    {
        for( Order o : trigger.new )
        {
            ...
            o.Name = o.Hidden_Auto_Number__c;
     ...
     // Added on 01/ 09 / 2016  
      List < order > OrderList = new List < Order > ();
        for(Order o : trigger.new){
            if(((trigger.isUpdate && trigger.isAfter) && (trigger.oldMap.get(o.ID).Order_Finalized_Date__c !=o.Order_Finalized_Date__c) ) || ((trigger.isInsert)&&(trigger.isAfter)&&(o.Order_Finalized_Date__c != null))){
